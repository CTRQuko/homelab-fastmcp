...
Doc canónico para entender qué pieza es qué en este framework y cómo
nombrar las tuyas sin colisionar. Léelo antes de empezar un plugin,
sobre todo si piensas publicarlo.
...
donde puede (regex del `plugin.name`, por ejemplo) y las sugiere donde
no (nombre del repo de un plugin terceiro).
...
`inventory`, `memory`, `loader`, `profile`, `skills`. Ninguno expone
tools MCP directamente. Son la maquinaria que consumen el router y
los plugins.
...
Los únicos tools que el framework expone al LLM por defecto,
definidas en `router.py`. Empiezan por `router_` y son parte del
contrato — cambiar sus nombres o signaturas rompe a cualquier cliente
que las use.
...
- `setup_<plugin>()` — dinámica, una por plugin con requirements
  pendientes.
- `skill_<nombre>()` / `agent_<nombre>()` — de discovery `.md`.

### Plugin

Un MCP server independiente cuyo manifest (`plugin.toml`) declara
...
clona (o symlinkea) bajo `plugins/<plugin-name>/` del framework para
que el router lo descubra.
...
- Nombre en el frontmatter `.md` → sanitizado a snake_case para el tool.
- Tool expuesto: `skill_<name>` o `agent_<name>`.
...
- **No hardcodees IPs, hostnames, rutas.** Todo lo específico del
  operador vive en `inventory/*.yaml` y se pide al router vía
  `core.inventory.get_hosts(type=..., tag=...)`.
- **No leas `os.environ` directamente para secretos.** Declara el
...
- **No escribas en `.env` ni en `~/.ssh/*` sin pasar por
  `router_add_credential`.** El vault centraliza la escritura.
- **No asumas SO** sin checkearlo. Si tu plugin es Windows-only o
  Linux-only, documenta el contrato (`requires.os = "windows"`,
  pendiente de schema si hace falta) o detéctalo al arrancar y falla
  limpio.
- **No importes `core/*` del framework** desde el código del plugin.
  Rompe el aislamiento y bloquea la publicación del plugin como repo
  independiente.
...
### Forma A — Python script standalone

Más simple. Útil para plugins sin dependencias externas a `fastmcp`.
Ver `examples/echo-plugin/` — es literalmente esto.
...
Cuando el plugin tiene `pyproject.toml` propio, dependencias externas
(requests, paramiko…) y quieres que `uv` gestione su venv.
...
ruta absoluta del directorio del plugin al mount-time. Útil cuando un
comando necesita el path explícito:
...
(plugin) y **MCP paralelo** (otro server MCP que el cliente configura
aparte del router):

### Cuándo plugin (subserver)

- La unidad se beneficia de compartir `inventory/`, `vault` y `audit`
  con el resto.
- Los tools se usan conversacionalmente junto con los tools del
...
Tener algo como plugin o como MCP paralelo no cambia la arquitectura
interna del framework: el cliente MCP ve ambos igual (una lista de
tools), solo cambia quién los hostea.
...
- [ ] Un smoke test que mounte el plugin y verifique que al menos un
      tool responde.
- [ ] Licencia clara.

## 7. Ejemplos vivos

- `examples/echo-plugin/` — el template canónico.
- `docs/operator-notes/cutover/manifests/` — manifests reales del
  despliegue del autor, útiles como referencia de casos más complejos
  (requirements, credenciales, `uv run`, `uvx`).

## 8. Errores comunes

- **Nombre del plugin con mayúsculas o guiones.** El regex
  `^[a-z][a-z0-9_]*$` lo rechaza. Usa snake_case.
- **`credential_refs = "PROXMOX_*"`** (string en vez de lista). TOML
  lo acepta pero el router falla cerrado: ninguna credencial
...
- **Importar `from core.secrets import ...` desde el código del
  ...
  credenciales via env, no vía import.
